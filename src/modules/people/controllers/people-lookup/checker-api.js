...
const checkerAPIController = {
  ...
  verifyEmail: (guest, attempt, emails, next) => {
    if (!guest.email || (guest.email && ((guest.verification && guest.verification !== 'deliverable') || !guest.confidence))) {
      ...
      let gue = guest;
      emailable.verify(emails[attempt].trim()).then(function (response) {
        if (response) {
          // Store in db
          let update = false;
          if (response.state !== 'undeliverable') {
            if (!gue.response && (response.state === 'risky' || response.state === 'deliverable'))
              update = true;
            else if (gue.response === 'risky' && response.state === 'deliverable')
              update = true;
          }
          if (update) {
            gue.email = response.email;
            gue.response = response.state;
          }
