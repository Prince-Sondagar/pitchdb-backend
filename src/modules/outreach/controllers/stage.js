...
const stageController = {
  getStagesBySequence: (sequenceId, callback) => {
    Stage.find({ sequence: sequenceId })
      .sort("-date")
      .exec((err, docs) => {
        if (err) callback(err);
  ...
  advanceStage: (sequenceId, newStage, callback) => {
    let updateObj = {};
    if (newStage === stageConstants.WAITING) callback();
    else {
      ...
      if (newStage === stageConstants.REPLIED)
  ...
  getLatestStagesBycategory: (category, userId, callback) => {
    OutreachSequence.find({
      userId: userId,
      currentStage: category,
      active: true
    })
      .sort("date")
      .populate("userPodcastId")
      .populate("userPodcastEpisodeId")
      .populate("userEventOrganizationId")
      .populate("userBusinessId")
      .populate("userMediaOutletId")
      .populate("userConferenceId")
      .populate("userGuestId")
      .populate("userSpeakerId")
      .exec((err, docs) => {
        if (err) callback(err);
        else {
          ...
          async.map(
            sequencesWithCategory,
            findLatestInSequenceByCategory,
            (err, result) => {
              if (err) callback(err);
  ...
  sendOutreachEmail: (user, stage, openId, callback) => {
    emailAccountController.getActiveEmailAccount(stage.userId, (err, account) => {
      checkEmailAccountValidity(
        ...
        (err, emailAccount) => {
          if (err) {
            ...
            emailLookUpController.verifyWithChecker(
              stage.sequence,
              (err, verification) => {
                if (err) callback(err);
                if (
                  ...
                  mailController.sendEmail(
                    ...
                    (err, data) => {
                      ...
                      //   emailsignature: '<p><img src="https://cdn4.iconfinder.com/data/icons/small-n-flat/24/user-group-128.png" alt="" /> <br/>\n' +
                      //     'rakesh patel<br/>react developer (IND)test data is here111bbb</p>',
                      //   from: 'pateanil82@hotmail.com',
                      //   to: 'rakeshpateltest84@gmail.com',
                      //   senderName: 'rakeshpatel test',
                      //   openId: 6155656419817f20107da4a2
                      // }
                      if (err) {
                        setInvalidEmailAccount(account._id);
                        callback(
                          new CustomError(
                            errorConstants.EMAIL_ACCOUNT_ERROR,
                            errorConstants.STATUS_EMAIL_ACCOUNT_ERROR
                          )
                        );
  ...
  sendOutreachEmailSecond: (user, stage, openId, callback) => {
    emailAccountController.getActiveEmailAccount(stage.userId, (err, account) => {
      checkEmailAccountValidity(account._id, (err, emailAccount) => {
        if (err) {
          setInvalidEmailAccount(account._id);
          callback(new CustomError("Email account error", 530));
        } else {
          emailLookUpController.verifyWithChecker(stage, (err, verification) => {
            if (err) callback(err);
            else {
              if (verification === "risky" || verification === "deliverable") {
                let emailData = stage.content;
                emailData.from = stage.emailFrom;
                emailData.to = stage.emailTo;
                emailData.senderName = user.name;
                emailData.openId = openId;
    ...
    checkEmailAccountValidity(
      ...
      (err, emailAccount) => {
        if (err) {
          ...
          if (emailAccount)
            mailController.getEmailConversation(
              stage.content,
              emailAccount,
              (err, thread) => {
                if (err) {
                  ...
                  stage.replyEmail = checkForReply(
                    emailAccount.network,
                    thread,
                    stage.sequence.currentMessage
                  );
                  callback(null, stage);
                }
              }
      ...
      stageController.checkBounce(stage.replyEmail.from, (err, bounced) => {
        if (err) {
          ...
          newStage = {
            ...
            sequence: stage.sequence._id,
            category:
              stage.sequence.emailFrom !== stage.replyEmail.from
                ? stageConstants.REPLIED
                : stageConstants.CONVERSED,
            content: {
  ...
  checkConversation: (sequence, thread, callback) => {
    ...
    if (!thread) {
      ...
      for (
        let i = sequence.stages.length - 1;
        i >= 0 && !latestReplyStage;
        i--
      ) {
        const stage = sequence.stages[i];
        if (
          stage.category === stageConstants.REPLIED ||
          stage.category === stageConstants.CONVERSED
        )
          latestReplyStage = stage;
      }
      if (!latestReplyStage) callback(null, sequence);
      else
        checkEmailAccountValidity(
          ...
          (err, emailAccount) => {
            if (err) {
              ...
              mailController.getEmailConversation(
                latestReplyStage.content,
                emailAccount,
                (err, fetchedThread) => {
                  if (err) {
                    setInvalidEmailAccount(
                      latestReplyStage.sequence.emailAccountId
                    );
                    callback(
                      new CustomError(
                        errorConstants.EMAIL_ACCOUNT_ERROR,
                    ...
                    protoStage.replyEmail = checkForReply(
                      emailAccount.network,
                      fetchedThread,
                      ...
                      createConversationStage(
                        ...
                        (err, newStage) => {
                          if (err) callback(err);
                            ...
                            stageController.checkConversation(
                              sequence,
                              fetchedThread,
                              callback
                            );
  ...
  if (conversation && currentMessage)
    switch (network) {
      case mailConstants.OUTLOOK:
        reply = outlookController.checkConversationForReply(
          conversation,
          currentMessage
        );
        break;
      case mailConstants.GMAIL:
        reply = gmailController.checkPayloadForReply(
          conversation,
          currentMessage
        );
        break;
...
const findLatestInSequenceByCategory = (outreachSequence, callback) => {
  Stage.findOne(
    { sequence: outreachSequence._id, category: outreachSequence.category },
    (err, doc) => {
      if (err) callback(err);
      else {
        if (doc) {
