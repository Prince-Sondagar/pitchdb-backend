...
const outreachSequenceController = {
  ...
  getOutreachDetail: (req, callback) => {
    outreachSequenceController.getById(
      req.params.id,
      req.decoded.userId,
      (err, outreachSequence) => {
        if (err) callback(err);
        else if (outreachSequence)
          if (req.query.withStages && req.query.withStages === "Y") {
            stageController.getStagesBySequence(
              req.params.id,
              (err, stages) => {
                if (err) callback(err);
                else {
                  ...
                  if (
                    outreachSequence.currentStage !== stageConstants.WAITING &&
                    outreachSequence.currentStage !== stageConstants.SENT &&
                    outreachSequence.currentStage !== stageConstants.OPENED &&
                    outreachSequence.currentStage !== stageConstants.CONVERSED &&
                    outreachSequence.currentStage !== stageConstants.BOOKED 
                  ) {
                    stageController.checkConversation(
                      ...
                      (err, convSequence) => {
                        if (err) callback(err);
                          ...
                          outreachSequenceController.updateOutreachSequence(
                            convSequence._id,
                            { currentMessage: convSequence.currentMessage },
                            err => {
                              if (err) callback(err);
                              else callback(null, convSequence);
                            }
                          );
                        }
                      }
                    );
                  } else callback(null, outreachSequence);
                }
              }
            );
          } else callback(null, outreachSequence);
        else callback(new CustomError("Not found", 404));
      }
    );
  },

  getById: (seqId, userId, callback) => {
    OutreachSequence.findOne({ _id: seqId, userId: userId })
  ...
  getBylistId: (req, callback) => {
    OutreachSequence.findOne({
      listItemId: req.params.listid,
      ...
      .populate("userSpeakerId")
      .lean()
      .exec((err, outreachSequence) => {
        if (err) {
        } else {
          if (outreachSequence) {
            stageController.getStagesBySequence(
              outreachSequence._id,
              (err, stages) => {
                if (err) callback(err);
                else {
                  outreachSequence.stages = stages;
  ...
  createOutreachSequenceContact: (outreachSequence, callback) => {
    let newOutreachSequence = new OutreachSequence(outreachSequence);
  ...
  deleteOutreachSequence: (outreachId, userId, callback) => {
    OutreachSequence.findOneAndRemove(
      { _id: outreachId, userId: userId },
      err => {
        if (err) callback(err);
        else stageController.deleteStagesFromSequence(outreachId, callback);
      }
    );
  },
