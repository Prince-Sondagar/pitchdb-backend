...
const stageActionsController = {
  stageSend: (req, callback) => {
    ...
    openController.createFromOutreachSequence(
      req.body.sequence._id,
      (err, open) => {
        if (err) callback(err);
        else
          stageController.sendOutreachEmail(
            req.decoded,
            req.body,
            open._id,
            (err, emailData) => {
              if (err) {
                if (err === "Verification error") {
                  callback(
                    new CustomError(
                      "Verification error",
                      errorConstants.STATUS_FAILED_SEND_LOOKUP
                    )
                  );
                } else {
                  callback(err);
  ...
  stageSendSecond: (req, callback) => {
    // Create an "open" document in the database to track if the recipient opens the sent email message
    openController.createFromOutreachSequence(req.body._id, (err, open) => {
      if (err) callback(err);
      else
        stageController.sendOutreachEmailSecond(
          req.decoded,
          req.body,
          open._id,
          (err, emailData) => {
            if (err) {
              ...
              // After the email has been sent, reduce the user's credit count by one
              afterSendCreateStageSecond(req, emailData, callback);
            }
          }
        );
    });
  ...
  stageReply: (req, callback) => {
    async.mapSeries(
      req.body,
      stageController.checkLatestInThread,
      (err, checkedStages) => {
        if (err) callback(err);
        else {
          // Filter out stages with errors
          // TODO: do something about checked stages with errors
          const filteredStages = checkedStages
            .filter(({ error }) => !error)
            .map(checkedStage => ({
              ...checkedStage,
              user: req.decoded
  ...
  stageConversation: (req, callback) => {
    stageController.createStage(
      {
        userId: req.decoded.userId,
        teamId: req.decoded.teamId,
  ...
  stageRestore: (req, callback) => {
    ...
    OutreachSequence.findOneAndUpdate(
      { _id: req.body._id, userId: req.decoded.userId },
      { currentStage: category },
      callback
    );
  },
  estimaterevenue: (req, callback) => {
    OutreachSequence.findOneAndUpdate(
      { _id: req.body._id, userId: req.decoded.userId },
      { estimatedrevenue: req.query.amount },
      callback
    );
  }
};
...
const afterSendCreateStageSecond = (req, data, callback) => {
  let reqStage = req.body;
  ...
  async.map(
    ...
    (err, checkedStages) => {
      if (err) {
        ...
        async.mapSeries(
          checkedStages.filter(({ error }) => !error),
          stageController.processReply,
          (err, processedStages) => {
            if (err) {
              winston.error(err);
              ...
              processedStages.forEach(stage => {
                if (stage) {
                  ...
                  if (stage.category === stageConstants.BOUNCED) bounced = true;
                }
              });
