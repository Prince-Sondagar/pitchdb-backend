...
// pm2 is acting up, so backend ulr is hardcoded here in order to get the pixel working
const BASE_BACK_URL = process.env.NODE_ENV === 'production' ? "https://app.pitchdb.com/api" : process.env.BACK_BASE_URL

const outlookController = {
  buildMailBody: emailData => {
    
    let signature;
    if(emailData.emailsignature !== undefined || emailData.emailsignature !== ""){
      signature = emailData.emailsignature;
    }else{
      signature = "";
    }
    ...
    return {
      message: {
        ...
        body: {
          ...
          //content: emailData.message + "\r\n" + "\r\n" + signature
          //   content: emailData.message + "\r\n" + signature ? emailData.message += "<br><br>-------------------------------------<br>\r\n\r\n":"" + signature + "\r\n"
          // // Insert invisible gif into email that will trigger the 'opens' endpoint whenever the user opens the email
          //   + '<img src="' + BASE_BACK_URL + '/opens/outreach-sequence/' + emailData.openId + '.gif" alt="image">'
        },
        toRecipients: [
          {
            emailAddress: {
              address: process.env.NODE_ENV === 'production' ? emailData.to : "ronstoryjr@gmail.com"
  ...
  */
  getSentMail: (emailData, emailAccount, callback) => {
    ...
    const toEmail = process.env.NODE_ENV === 'production' ? emailData.to : "ronstoryjr@gmail.com";
