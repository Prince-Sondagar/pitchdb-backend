...
const searchController = {
  saveSearch: (user, searchObj) => {
    ...
    if (searchObj.filters.exclude)
      searchObj.filters.exclude = searchController.parseKeywords(
        searchObj.filters.exclude
      );
  ...
  parsePodcastSearch: searchObj => {
    if (searchObj.filters.genres && searchObj.filters.genres.length > 0)
      searchObj.filters.genres = searchController.parseKeywords(
        searchObj.filters.genres
      );
    else delete searchObj.filters.genres;
    if (searchObj.filters.publishedBefore)
      searchObj.filters.publishedBefore = new Date(
        searchObj.filters.publishedBefore
      );
    if (searchObj.filters.publishedAfter)
      searchObj.filters.publishedAfter = new Date(
        searchObj.filters.publishedAfter
      );
  },
  ...
  getSearches: (req, callback) => {
    ...
    Search.find({})
      .sort("-date")
      .skip(req.query.page ? req.query.page * SEARCH_PAGE_SIZE : 0)
  ...
  getcurrentuser: (req, callback) => {
    User.findById(req.decoded.userId).exec((err, users) => {
      if (err) callback(err);
