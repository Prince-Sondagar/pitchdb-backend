...
const listItemController = {
  create: (req, callback) => {
    ...
    items.forEach((item) => {
      ...
      if (user.teamId) item.teamId = user.teamId;
    ...
    async.map(
      itemObjects,
      userItemController.findOrCreateItem,
      (err, itemObjects) => {
        if (err) {
          callback(err);
        } else {
  ...
  countSummary: (req, callback) => {
    ...
    async.parallel(
      [
        (callback) =>
          listItemController.countItems(listId, "podcast", callback),
        (callback) =>
          listItemController.countItems(listId, "episode", callback),
        (callback) =>
          listItemController.countItems(listId, "eventOrganization", callback),
        (callback) =>
          listItemController.countItems(listId, "business", callback),
        (callback) =>
          listItemController.countItems(listId, "mediaOutlet", callback),
        (callback) =>
          listItemController.countItems(listId, "conference", callback),
        (callback) => listItemController.countItems(listId, "guest", callback),
        (callback) =>
          listItemController.countItems(listId, "speaker", callback),
      ],
      (err, counts) => {
        if (err) callback(err);
        else {
  ...
  get: (req, callback) => {
    ...
    ListItem.find(queryObj)
      .populate(getFieldByType(type))
      .sort("-date")
      .limit(LISTS_ITEMS_PAGE_SIZE)
      .skip(req.query.page ? req.query.page * LISTS_ITEMS_PAGE_SIZE : 0)
      .exec(callback);
  },

  getcontactdata: (req, callback) => {
    let getids = req.params.id;
    let ids = new Array();
    ids = getids.split(",");
    console.log("getids",req?.decoded, ids)
    ListItem.find({ listId: { $in: ids } })
      .populate("userPodcast")
      .populate("userPodcastEpisode")
      .populate("userEventOrganization")
      .populate("userBusiness")
      .populate("userSpeaker")
      .populate("userMediaOutlet")
      .populate("userConference")
      .populate("userGuest")
      .sort("-date")
      .exec(callback);
  },
// 648e391e55c53c1d4af7de7e,64f758f303be9f6fd61adcb3
  count: (req, callback) => {
    listItemController.countItems(
      req.params.id,
      req.query.type,
      (err, count) => {
        if (err) callback(err);
    ...
    counterController.get(
      ...
      (err, counter) => {
        ...
        if (counter && counter.remaining > 0) {
          ...
          emailAccountController.getActiveEmailAccount(
            req.decoded.userId,
            (err, emailAccount) => {
              if (err) {
                return callback(err);
              ...
              async.map(
                itemsList,
                listItemController.connectContact,
                (err, results) => {
                  if (err) return callback(err);
          ...
          callback(
            new CustomError(
              ...
              errorConstants.STATUS_PAYMENT_REQUIRED
            )
          );
      }
    );
  ...
  connectContact: (item, callback) => {
    listItemContact.getContactData(item, (err, email) => {
      emailable.verify(email.trim()).then(function (response) {
        if (response.state === "undeliverable") {
          listItemContact.updateContactData(item)
          callback(
            new CustomError(
              "Not Validated!",
              530
          ...
          listItemContact.addContactData(item, (err, email) => {
            ...
            outreachSequenceController.createOutreachSequence(
              item,
              (errs, sequence) => {
                if (errs) {
                  return callback(errs);
                } else {
                  creditController.useCredit(
                    item.userId,
                    item.teamId,
                    sequence._id,
                    (err) => {
  ...
  CreateSqquencefromcontact: (req, callback) => {
    counterController.get(
      req.decoded.userId,
      req.decoded.teamId,
      (err, counter) => {
        ...
        if (counter && counter.remaining > 0) {
          ...
          emailAccountController.getActiveEmailAccount(
            ...
            (err, emailAccount) => {
              ...
              itemsList.userId = user.userId;
              if (user.teamId) itemsList.teamId = user.teamId;
              itemsList.emailFrom = emailAccount.email;
              itemsList.emailAccountId = emailAccount._id;

              listItemContact.addContactData(itemsList, (err, email) => {
                if (err) return callback(err);

                if (!email) return callback();

                itemsList.emailTo = email;
                itemsList.active = true;

                outreachSequenceController.createOutreachSequenceContact(
                  itemsList,
                  (err, sequence) => {
                    if (err) {
                      callback(err);
  ...
  getSequence: (req, callback) => {
    OutreachSequence.findOne(
      { userId: req.decoded.userId, listItemId: req.params.listItemId },
      callback
    );
  },
...
const setUserItemAddParameters = (type, itemObject) => {
  switch (type) {
    ...
    // thisis
    case "speaker":
      itemObject.model = UserSpeaker;
      itemObject.itemIdKey = "speakerId";
      itemObject.idKey = "_id";
      break;
    // endisa
    case "mediaOutlet":
...
const setListItemObjId = (type, listItem, userItemId) => {
  switch (type) {
    ...
    case "business":
      ...
      break;
    case "speaker":
      listItem.userSpeaker = userItemId;
      break;
    case "mediaOutlet":
...
const setListItemQuery = (queryObj, type) => {
  ...
  switch (type) {
    ...
    case "business":
      ...
      break;
    case "speaker":
      queryObj.userSpeaker = existsExpression;
      break;
    case "mediaOutlet":
...
const updateUserBusinessOwner = (oldUserId, newUserId, callback) => {
  UserBusiness.updateMany(
    { userId: oldUserId },
    { userId: newUserId, oldUser: oldUserId },
    callback
  );
};

// speakerstart
const updateUserSpeakerOwner = (oldUserId, newUserId, callback) => {
  UserSpeaker.updateMany(
    { userId: oldUserId },
    { userId: newUserId, oldUser: oldUserId },
    callback
  );
...
const updateUserEvenOrgOwner = (oldUserId, newUserId, callback) => {
  UserEventOrganization.updateMany(
    { userId: oldUserId },
    { userId: newUserId, oldUser: oldUserId },
    callback
  );
