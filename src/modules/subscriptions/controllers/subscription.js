...
const subscriptionController = {
  ...
  addPlan: (req, callback) => {

    const plandata = req.body;

    stripe.plans.create(plandata.requestBody, (err, customer) => {
      if (err) {
        callback(err)
      } else {
  ...
  addSubscription: (req, callback) => {
    const user = req.decoded;
    const paymentData = req.body;

    const { planId } = paymentData;
    stripeCommon.retriveOrCreateStripeCustomer(paymentData, (err, customer) => {
      if (err) {
        callback(err)
      } else {
        subscriptionController.getUserCurrentSubscriptionNew(paymentData, (err, userSubscription) => {
          if (err) {
          } else {
            if (userSubscription) {
                            ...
                            Subscription.findOneAndUpdate({ _id: userSubscription._id },
                              {
                                ...
                                if (err) callback(err)
                                  ...
                                  if (updatedSubscription.credits === Number.POSITIVE_INFINITY) {
                                    subscriptionController.addUnlimitedSubscriptionCredits(user, updatedSubscription, (err) => {
                                      if (err) callback(err)
                                      else callback(null, updatedSubscription)
                                    })
                                  }
                                  else if (userSubscription.credits === Number.POSITIVE_INFINITY) {
                                    subscriptionController.manageUnlimitedSubscriptionCredits(user, updatedSubscription, (err) => {
                                      if (err) callback(err)
                                      else callback(null, updatedSubscription)
                                    })
                                  }
                                  else if (userSubscription.credits < updatedSubscription.credits)
                                    subscriptionController.addSubscriptionCredits(user, updatedSubscription, (err) => {
                                      if (err) callback(err)
                                      else callback(null, updatedSubscription)
                                    })
                                  else
                                    subscriptionController.removeSubscriptionCredits(user, updatedSubscription, (err) => {
                                      if (err) callback(err)
                                      else callback(null, updatedSubscription)
                                    })
                                }
              ...
              stripe.subscriptions.create({
                ...
                if (err) {
                  ...
                  subscriptionController.getNewPlanById(planId, (err, plan) => {
                    if (err) callback(err)
                    else {
  ...
  getNewPlanById: (planId, callback) => {
    stripe.plans.retrieve(planId, callback)
  },
  ...
  getUserCurrentSubscriptionNew: (req, callback) => {
    Subscription.findOne({ userId: req.userid, status: 'active' }).sort('-lastRenewWal').exec(callback)
  },
  ...
  addSubscriptionCredits: (user, subscription, callback) => {
    ...
    Credit.countDocuments({ userId, dateLastRefund: null, dateConsumed: null, subscription: true }, (err, count) => {
      if (err) {
        ...
        const amountToInsert = subscription.credits
        if (amountToInsert <= 0) {
          callback()
        }
        else {
          ...
          Credit.insertMany(creditsArray, err => {
            if (err) {
              ...
              Counter.updateOne({ userId: userId }, { remaining: (subscription.credits + count) }, (err) => {
                if (err) {
                  backErrorController.persistError({
                    module: "subscription",
  ...
  addUnlimitedSubscriptionCredits: (user, subscription, callback) => {
    ...
    Counter.findOneAndUpdate({ userId }, { remaining: Number.POSITIVE_INFINITY }, (err) => {
      if (err) {
        backErrorController.persistError({
          module: "subscription",
          message: `Error updating counter for u: ${userId} t: ${user.teamId}  subId: ${subscription._id}`
