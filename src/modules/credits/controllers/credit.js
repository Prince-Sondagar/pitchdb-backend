...
module.exports = {
  ...
  addCredits: (userId, teamId, credits, callback) => {
    let creditsArray = [];
    ...
    Credit.insertMany(creditsArray, err => {
      if (err) {
        ...
        else filterObj.userId = userId;
        Counter.update(filterObj, { $inc: { remaining: credits.amount } }, (err) => {
          if (err) {
            backErrorController.persistError({
              module: "adding gift credits",
  ...
  obtainBundle: (userId, teamId, bundle, paymentId, callback) => {
    ...
    Credit.insertMany(creditsArray, err => {
      if (err) {
        ...
        Counter.update(filterObj, { $inc: { remaining: bundle.amount } }, (err) => {
          if (err) {
            backErrorController.persistError({
              module: "buying bundle",
  ...
  useCredit: (userId, teamId, outreachSequenceId, callback) => {
    ...
    Counter.findOne(filterObj, null, function (err, counter) {
...
const updateCounterUseCredit = (userId, teamId, outreachSequenceId, callback) => {
  ...
  Counter.updateOne(updateOBj, { $inc: { used: 1, remaining: -1 } }, (err) => {
    if (err) {
      backErrorController.persistError({
        module: "using credit",
        message: "Error updating counter for u:" + userId + " t:" + teamId + " outre:" + outreachSequenceId
