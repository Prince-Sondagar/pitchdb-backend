...
const { PODCAST_SEARCH } = require('../../modules/common/constants/users/permissions')

router.all('/*', routeInterceptor.verifyToken);
router.all('/*', routeInterceptor.verifySessionValidity);
router.all('/*', routeInterceptor.verifyPrivileges({ requiredPrivileges: [PODCAST_SEARCH] }));
...
router.get('/:id/allepisodes', routeInterceptor.useCache(100), (req, res, next) => {
  podcastController.getAllEpisodes(req, (err, episodes) => {
    handleStandard(req, res, err, episodes, next);
  })
})
