...
router.put('/items/contactnew', (req, res, next) => {
  listItemController.CreateSqquencefromcontact(req, (err, result) => {
    handleStandard(req, res, err, result, next);
  })
});

router.get('/:id/contactitems', (req, res, next) => {
  listItemController.getcontactdata(req, (err, result) => {
    handleStandard(req, res, err, result, next);
  })
});
