...
router.put('/:id/delete', (req, res, next) => {
  emailAccountController.DeleteAccount(req.decoded.userId, req.params.id, (err, data) => {
    handleStandard(req, res, err, data, next);
  })
})
...
router.post('/addemailsignature', (req, res, next) => {
  emailAccountController.addEmailsignature(req, (err, data) => {
    handleStandard(req, res, err, data, next);
  })
})
