...
router.post('/addSubscription', (req, res, next) => {
  subscriptionController.addSubscription(req, (err, result) => {
    handleStandard(req, res, err, result, next);
  })
})

router.post('/addPlan', (req, res, next) => {
  subscriptionController.addPlan(req, (err, result) => {
    handleStandard(req, res, err, result, next);
  })
})

router.get('/getStripePlansdata', (req, res, next) => {
  subscriptionController.getStripePlansdata((err, plans) => {
    handleStandard(req, res, err, plans, next);
  })
})
