...
router.put("/:id/credits", (req, res, next) => {
  adminCreditController.addCredits(
    req.params.id,
    req.body,
    (err, creditsData) => {
...
router.get("/user-privelege", (req, res, next) => {
  adminUserController.getPrivilegelist(req, (err, data) => {
    handleStandard(req, res, err, data, next);
  });
});
...
router.put("/:id/remove-privilege", (req, res, next) => {
  adminUserController.removePrivilege(
    req.params.id,
    req.query.privilege,
    err => {
      handleStandard(req, res, err, null, next);
...
router.put("/:id/add-assign-template", (req, res, next) => {
  adminUserController.adminAssigntemplate(req, err => {
    handleStandard(req, res, err, null, next);
  });
});
